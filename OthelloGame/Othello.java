...
public class Othello {
    ...
    private Vector2<Integer> nextMoveCoordinates = new Vector2<Integer>();
    private static Othello GameInstance;
    ...
    public static Othello getInstance() {
        if (GameInstance == null) {
            GameInstance = new Othello();
        }
        return GameInstance;
    }
    public static void main(String[] args) {
        Othello game = Othello.getInstance();
        game.startGame();
    ...
    private void setupGame() {
        ...
        if (myGameBoard.rows <= 0 || myGameBoard.columns <= 0) {
            myGameBoard.rows = myGameBoard.columns = 8; // Default size
        }
    ...
    private void getPlayerInput() {
        ...
        if (isValidMove(nextMoveCoordinates.x, nextMoveCoordinates.y)) {
            myGameBoard.BoardArray[nextMoveCoordinates.x][nextMoveCoordinates.y] =
                isPlayer1 ? BoardPiecesEnum.black : BoardPiecesEnum.white;
    ...
    private boolean isValidMove(int x, int y) {
        return ((x >= 0) && (x < myGameBoard.rows) && (y >= 0) && (y < myGameBoard.columns && myGameBoard.BoardArray[x][y] == BoardPiecesEnum.none));
    }
